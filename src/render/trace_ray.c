...
void  render_sphere(t_ray_var *v, t_ray ray, int recurse, t_rt *rt)
{
	...
	if (v->its.c_sphere->is_chess)
	{
		...
		v->color = chessboard(ray, v->even_color, v->odd_color, v->its.closest_t
		, v->its.c_sphere->is_chess);
...
void  render_plane(t_ray_var *v, t_ray ray, int recurse, t_rt *rt)
{
	...
	if (v->its.c_plane->is_chess)
	{
		...
		v->its.closest_t, v->its.c_plane->is_chess);
		v->color = multiply_vect(lighting(v->point, v->normal,
		rt, v->its.c_plane->specular, v->view), v->color);
	}
	else
		v->color = multiply_vect(lighting(v->point, v->normal,
		rt, v->its.c_plane->specular, v->view), v->its.c_plane->color);
...
t_vect	trace_ray(t_rt *rt, t_ray ray, int recurse)
{
	...
	v.its.min = ray.min;
	v.its.max = ray.max;
	closest_intersection(ray.origin, rt, ray.dir, &v.its);
	if (v.its.hit_flag == -1)
	...
	{
		v.color.x += (rt->ambient.color.x * rt->ambient.ratio) * v.color.x / 255;
		v.color.y += (rt->ambient.color.y * rt->ambient.ratio) * v.color.y / 255;
		v.color.z += (rt->ambient.color.z * rt->ambient.ratio) * v.color.z / 255;
	}
	return (v.color);
}