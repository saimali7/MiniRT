...
static double	get_disc(t_vect origin, t_vect direction, t_cylinder *cylinder)
{
...
float	get_root(t_vect origin, t_vect direction, t_cylinder *cylinder, int mode, float sqrtd)
{
...
{
    float	discriminant = get_disc(origin, direction, cylinder);
    if (discriminant < 0)
		return (0);
    intsect[0] = get_root(origin, direction, cylinder, 1, sqrt(discriminant));
    float   h = dot_product_vect(origin, cylinder->orient) + intsect[0] * dot_product_vect(direction, cylinder->orient)
		- dot_product_vect(cylinder->coord, cylinder->orient);
    if (h < 0 || h > cylinder->height || intsect[0] < intersect->min || intersect->max < intsect[0])
	{
		intsect[0] = get_root(origin, direction, cylinder, 2, sqrt(discriminant));
		h = dot_product_vect(origin, cylinder->orient) + intsect[0] * dot_product_vect(direction, cylinder->orient)
			- dot_product_vect(cylinder->coord, cylinder->orient);
		if (h < 0 || h > cylinder->height || intsect[0] < intersect->min || intersect->max < intsect[0])
			return (0);
	}
